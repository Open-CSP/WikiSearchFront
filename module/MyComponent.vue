<template>
  <div id="app" :class="dragging">
    ...
    <select @change="getmenu"  class="form-control"  ><!-- v-if="!namespace" -->
      <option disabled selected>Select a menu</option>
      <option v-for="menu in menus" :value="menu">{{menu}}</option>
    </select>
    <div v-if="selectedmenu" style="display: grid;grid-template-columns: 2fr 1fr;">
      ...
      <div class="menu-form" v-show="$store.getters.selected">
        ...
        <label @click.prevent="showsearch" >
          Icon
          <input v-show="!ifsearch"  type="text" @change="inputicon"  :value="Icon"  class="form-control"/>
        </label>
        <div v-show="ifsearch" id="fasearch" >
